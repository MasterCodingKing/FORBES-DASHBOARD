...
const MonthlyProjectionReport = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      {/* Report Content */}
      <div id="report-content" ref={reportRef} className="bg-white rounded-xl shadow-lg p-6 max-w-3xl mx-auto">
        ...
        <table className="w-full text-sm border-collapse">
          ...
          <tbody>
            {serviceBreakdown?.breakdown?.map((s, idx) => (
              <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                <td className="border border-gray-400 px-4 py-2 italic">{s.departmentName}</td>
                <td className="border border-gray-400 px-4 py-2 text-right">
                  {formatNum((monthlyRevenue?.yearTotal || 0) / selectedMonth / (serviceBreakdown?.breakdown?.length || 1))}
                </td>
                <td className="border border-gray-400 px-4 py-2 text-right">{formatNum(s.revenue * 1.35)}</td>
          ...
          <tfoot>
            ...
            <tr className="font-bold border-t-2 bg-gray-50">
              ...
              <td className="border border-gray-400 px-4 py-3 text-right text-blue-600">35%</td>
              <td className="border border-gray-400 px-4 py-3 text-right">-</td>
            </tr>
          </tfoot>
        </table>
