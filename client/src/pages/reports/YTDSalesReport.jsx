...
const YTDSalesReport = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [revenueResp, prevRevenueResp] = await Promise.all([
  ...
  }, [monthlyRevenue, prevYearRevenue, selectedMonth]);

  const handleExport = async (format) => {
    const filename = `YTD-Sales-Comparative-${months[selectedMonth - 1]}-${selectedYear}`;
    
    if (format === 'png') {
      await exportToPNG('report-content', filename);
    } else if (format === 'pdf') {
      await exportToPDF('report-content', filename, 'Year to Date Comparative - Sales Report');
    } else if (format === 'excel') {
      const data = ytdData.revenueMonths?.map((m, i) => ({
        Month: m.monthName,
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800">YEAR TO DATE COMPARATIVE - SALES</h1>
            <p className="text-gray-500">Compare year-to-date sales performance</p>
          </div>
        </div>
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              <tr>
                ...
                <td className="border border-gray-400 px-2 py-1 text-right font-bold">{formatNum(ytdData.previousRevenue)}</td>
              </tr>
              <tr>
                ...
                <td className="border border-gray-400 px-2 py-1 text-right font-bold">{formatNum(ytdData.currentRevenue)}</td>
              </tr>
            </tbody>
          </table>
        </div>
