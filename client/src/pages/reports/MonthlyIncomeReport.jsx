...
const MonthlyIncomeReport = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const [incomeResp, breakdownResp] = await Promise.all([
  ...
  const handleExport = async (format) => {
    const filename = `Monthly-Income-Trend-${months[selectedMonth - 1]}-${selectedYear}`;
    
    if (format === 'png') {
      // Hide detailed table and show simple table for export
      document.getElementById('detailed-table-income').style.display = 'none';
      document.getElementById('export-table-income').style.display = 'block';
      await exportToPNG('report-content', filename);
      // Restore original display
      document.getElementById('detailed-table-income').style.display = 'block';
      document.getElementById('export-table-income').style.display = 'none';
    } else if (format === 'pdf') {
      // Hide detailed table and show simple table for export
      document.getElementById('detailed-table-income').style.display = 'none';
      document.getElementById('export-table-income').style.display = 'block';
      await exportToPDF('report-content', filename, 'Monthly Income Trend Report');
      // Restore original display
      document.getElementById('detailed-table-income').style.display = 'block';
      document.getElementById('export-table-income').style.display = 'none';
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800">MONTHLY INCOME TREND</h1>
            <p className="text-gray-500">Monthly income analysis report</p>
          </div>
        </div>
      ...
      <div id="report-content" ref={reportRef} className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div id="export-table-income" className="overflow-x-auto" style={{display: 'none'}}>
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr>
                <th className="border border-gray-300 bg-gray-200 px-4 py-2 text-left font-semibold">Month</th>
